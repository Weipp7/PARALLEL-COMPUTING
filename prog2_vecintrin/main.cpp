...
void clampedExpVector(float *values, int *exponents, float *output, int N)
{
  ...
  __cs149_vec_float result, tmp;
  __cs149_vec_int zero_int = _cs149_vset_int(0), one_int = _cs149_vset_int(1);
  __cs149_vec_float zero_float = _cs149_vset_float(0.f), max_float = _cs149_vset_float(9.999999f);
  __cs149_mask maskAll, maskIsNegative, maskIsNotNegative;
  int num = 0, group = N / VECTOR_WIDTH + (N % VECTOR_WIDTH ? 1 : 0);
  float *Value = new float[group * VECTOR_WIDTH];
  int *Exponent = new int[group * VECTOR_WIDTH];
  float *Output = new float[group * VECTOR_WIDTH];
  memset(Value, 0, sizeof(float) * group * VECTOR_WIDTH);
  memset(Exponent, 0, sizeof(int) * group * VECTOR_WIDTH);
  memset(Output, 0, sizeof(int) * group * VECTOR_WIDTH);
  for (int i = 0; i < N; i++)
  {
    Value[i] = values[i];
    Exponent[i] = exponents[i];
  }

  for (int i = 0; i < group * VECTOR_WIDTH; i += VECTOR_WIDTH)
  {
    // All ones
    maskAll = _cs149_init_ones();
    ...
    // Load vector of values from contiguous memory addresses
    _cs149_vload_float(x, Value + i, maskAll);  // float x = values[i];
    _cs149_vload_int(y, Exponent + i, maskAll); // int y = exponents[i];

    // Set mask according to predicate
    _cs149_veq_int(maskIsNegative, y, zero_int, maskAll); // if (y == 0){

    // Execute instruction using mask ("if" clause)
    _cs149_vset_float(result, (1.f), maskIsNegative); //  output[i] = 1.f;

    // Inverse maskIsNegative to generate "else" mask
    maskIsNotNegative = _cs149_mask_not(maskIsNegative); // } else {

    // Execute instruction ("else" clause)
    _cs149_vload_float(result, Value + i, maskIsNotNegative); //  float result = x;
    _cs149_vsub_int(count, y, one_int, maskIsNotNegative);    // int count = y - 1;
    _cs149_vlt_int(maskIsNotNegative, zero_int, count, maskIsNotNegative);
    // _cs149_vstore_int(&num, count, maskAll);
    bool exp = false;
    for (int k = 0; k < VECTOR_WIDTH; k++)
    {
      exp = exp || maskIsNotNegative.value[k];
    }
    while (exp)
    {                                                            // while (count > 0)
      _cs149_vmult_float(result, result, x, maskIsNotNegative);  // result *= x;
      _cs149_vsub_int(count, count, one_int, maskIsNotNegative); // count--;
      _cs149_vlt_int(maskIsNotNegative, zero_int, count, maskIsNotNegative);
      exp = false;
      for (int k = 0; k < VECTOR_WIDTH; k++)
      {
    ...
    _cs149_vlt_float(maskIsNotNegative, max_float, result, maskIsNotNegative); // if (result > 9.999999f){
    _cs149_vset_float(result, (9.999999f), maskIsNotNegative);                 // result = 9.999999f;
    _cs149_vstore_float(Output + i, result, maskAll);
  }
  for (int i = 0; i < N; i++)
  {
